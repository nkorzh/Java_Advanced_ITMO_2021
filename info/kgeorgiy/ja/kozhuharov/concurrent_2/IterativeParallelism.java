...
public class IterativeParallelism implements ListIP {
    ...
    private <T, U> U distribute(final int threads,
        ...
        if (mapper != null) {
            final List<Stream<? extends T>> subtasks = new ArrayList<>(threadsAmount);
            for (int i = 0; i < threadsAmount; i++) {
                subtasks.add(getThreadPart.apply(i).stream());
            }
            return collectFunc.apply(mapper.map(calcFunc, subtasks).stream());
        }

        final List<Thread> threadList = new ArrayList<>(threadsAmount);
